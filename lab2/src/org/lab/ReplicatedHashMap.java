 ...
 */
public class ReplicatedHashMap extends ReceiverAdapter {
    ...
    /** Swaps referenced value  with new value*/
    public void _compareAndSwap(String key, double referenceValue, double newValue) {
        if (stocks.get(key) == referenceValue) {
            synchronized (stocks) {
                stocks.put(key, newValue);
                System.out.printf("-- swapped stock %s with value %s to  %s\n",key, referenceValue, newValue);
            }

        } else
            System.out.printf("-- stock %s with value %s doesn't match %s\n",key, stocks.get(key), referenceValue);
    }
    ...
    private void start(String props) throws Exception {
        ...
        while(true) {
            int c=Util.keyPress("[1] Show stocks [2] Get quote [3] Set quote [4] Remove quote [5] compare and swap [x] Exit");
            try {
                switch(c) {
                    case '1':
                        showStocks();
    ...
    private void compareAndSwap() throws Exception {
        String ticker, val, newVal;
        ticker=readString("Symbol");
        val=readString("Reference value");
        newVal=readString("New value");
        RspList<Void> rsps=disp.callRemoteMethods(null,"_compareAndSwap",new Object[]{ticker,Double.parseDouble(val),Double.parseDouble(newVal)},
    ...
    public static void main(String[] args) throws Exception {
        ...
        new ReplicatedHashMap().start(props);
    }
}
